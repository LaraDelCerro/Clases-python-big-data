...
#paso 1: recibimos un texto como parámetro dentro de la fcón.
#paso 2: convertir en minúsculas
...
#def invertir_texto(texto): #opción 1
#    texto_invertido = ''
#    for caracter in texto:
...
def invertir_texto(texto):
    texto_invertido = texto [::-1] #[::]recorre el texto entero, el -1 indica que va hacia atás


def es_palindromo(texto):
    texto_final = texto.lower()
    texto_final_sin_acentos = quitar_acentos /(texto_final)
    texto_final_sin_acentos_sin_espacios = quitar_espacios (texto_final_sin_acentos)
    texto_invertido = invertir_texto(texto_final_sin_acentos)
    if texto_final_sin_acentos_sin_espacios == texto_invertido:

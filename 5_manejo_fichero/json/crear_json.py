...
def crear_json(carpeta, nombre, datos):
    os.makedirs(carpeta, exist_ok= True) #si la carpeta ya existe no la crea
    fichero = open(f'./{carpeta}/{nombre}', 'w', encoding='UTF-8')
    json.dump(datos, nombre, ensure_ascii=False, indent= 4)
    fichero.close()

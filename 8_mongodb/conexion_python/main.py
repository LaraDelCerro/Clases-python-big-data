...
[x]. Salir    
    ...
    print(menu)
    print('=' * 30)
    option = input('Dime qué opción seleccionas: ')
    print(option)
    ...
    if option == '1':
       ...
       print(df[['lugar', 'magnitud', 'fechaHora']])
        ...
        lugar = input('Dime de qué lugar quieres el sismo: ')
        ...
        try:
            ...
            print('la magnitud y y profundidad son numéricas')
            init()   
    elif option == '4':
        ...
        id_sismo = input('Dime el ID del sismo que quieres actualizar: ')
        sismo = get_by_id(id_sismo)
        print(f"Lugar actual: {sismo['lugar']} , magnitud actual: {sismo['magnitud']}")

        actualizar = input('¿Deseas actualizar este sismo (sí o no)?: ')
        if actualizar == 'no':
            print('No se ha actualizado nada')
            init()
        print('Si no quieres actualizar un campo, dale al Enter')
        datos_actualizar = {}
        ...
        try:
            ...
            print('La magnitud tiene que ser número')
